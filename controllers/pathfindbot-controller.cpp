	...
	*/
	...
	switch (m_eMovementState){
		case STATIONARY: {
			...
			m_dLeftWheelSpeed = 0;
			m_dRightWheelSpeed = 0;
			
			CVector2 vecToTarget (m_cTarget - currentPos);
			...
			m_nTimes      = (int)(diffVecAngle.GetValue() / m_dMaxRotnSpeed);
			m_dRemainDisp = fmod (diffVecAngle.GetValue(),  m_dMaxRotnSpeed);
			
			if (m_nTimes == 0 && Abs(m_dRemainDisp) < (CRadians::PI / 90).GetValue()){
				// our angle is close enough, now we check if we need to move forward
				m_nTimes      = (int) (vecToTarget.Length() / m_dMaxVelocity);
				m_dRemainDisp = fmod (vecToTarget.Length(),  m_dMaxVelocity);
				
				if (m_nTimes == 0 && Abs(m_dRemainDisp) < 0.1){
					LOG << "Arrived at local target?" << std::endl;
				} else {
					m_eMovementState = MOVING_FORWARD;
				}
			} else {
				m_eMovementState = ROTATING;
			}
			break;
		} case ROTATING: {
			...
			if (m_nTimes != 0){
				...
				double wheelVelDiff = m_dMaxRotnSpeed / FOOTBOT_INTERWHEEL_DIST;
				double wheelVelocity = wheelVelDiff / 2;
				if (m_nTimes > 0){
					m_dLeftWheelSpeed  = -wheelVelocity;
					m_dRightWheelSpeed =  wheelVelocity;
					m_nTimes -= 1;
				} else if (m_nTimes < 0){
					m_dLeftWheelSpeed  =  wheelVelocity;
					m_dRightWheelSpeed = -wheelVelocity;
				...
				// states
				// angVel = (m_dRightWheelSpeed - m_dLeftWheelSpeed) / FOOTBOT_INTERWHEEL_DIST
				double angVel = m_dRemainDisp / m_dSimSecPerTick;
			...
			// double temp   = (m_dRightWheelSpeed + m_dLeftWheelSpeed) / 2;
			if (m_nTimes > 0){
				// go the max velocity
				m_dLeftWheelSpeed = m_dRightWheelSpeed = m_dMaxVelocity;
				m_nTimes -= 1;
			} else {
				// calculate velocity needed to cover the remaining 
				// displacement/distance before changing states
				double velocity = m_dRemainDisp;// / m_dSimSecPerTick;
				m_dRightWheelSpeed = m_dLeftWheelSpeed = velocity;
				m_eMovementState = STATIONARY;
			}
			break;
		}
	...
	m_pcWheels->SetLinearVelocity(
		100 * m_dLeftWheelSpeed,  // convert from m/s to cm/s
		100 * m_dRightWheelSpeed
	);
	 ...
	 */
	double angVel = (m_dRightWheelSpeed - m_dLeftWheelSpeed) / FOOTBOT_INTERWHEEL_DIST;
	double temp   = (m_dRightWheelSpeed + m_dLeftWheelSpeed) / 2;
	CVector2 linVel (temp * ARGOS_COS(angleAboutZ.GetValue()), temp * ARGOS_SIN(angleAboutZ.GetValue()));
	
	LOG << "lv = " << linVel.Length() << " m/s, av = " << angVel << " rad/s" << std::endl;
